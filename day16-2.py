...
with open('input/day16.txt') as f:
    ...
    for l in f.readlines():
        label, flow, *dsts = re.findall(r'[A-Z]{2}|[0-9]+',l)
        caves.append(Cave(label, tuple(dsts), int(flow)))

paths = complete_paths(caves, 30)
print(max([p.pressure for p in paths]))


t1 = timer()
paths = complete_paths(caves, 26, stopping=True)
print(max_pressure_dual_paths(paths))
print(timer()-t1)
